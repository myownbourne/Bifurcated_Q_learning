BQL_4 <- function(S1,S2,A1,A2,y,J1s,J2s,L1,L2,C1s,CJ1s,C2s,CJ2s,Learners,Learners_ps) 
{
  n = nrow(S1)
  cf = sample(1:n,n/2)
  ...
  X2_b = cbind(A1,S2_b)
  g_hat1 = SuperLearner(Y = A2[cf], X = as.data.frame(X2_b[cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
  g_hat2 = SuperLearner(Y = A2[-cf], X = as.data.frame(X2_b[-cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
  ...
  g_hat31 = SuperLearner(Y = A1[cf], X = as.data.frame(SJ1f_b[cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
  g_hat32 = SuperLearner(Y = A1[-cf], X = as.data.frame(SJ1f_b[-cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
  for(k1 in 1:length(J1s)){
...
BQL_regimes_4 <- function(alphas,betas,gammas,deltas,S1_new,J1s,J2s,L1,L2,C1s,CJ1s,C2s,CJ2s,parms,k)
{
  n_test = nrow(S1_new)
  ...
  beta_Y1 = parms[[1]]; beta_R = parms[[2]];  beta_SR = parms[[3]]
  R_new = S2_new - S1_new - as.vector(a1_hat);
  SR_new = cbind(S1_new,a1_hat,R_new)
  y_new = SR_new %*% beta_SR + a1_hat * (S1_new %*% beta_Y1 + k * (S1_new[,1]^2-1)) + a2_hat * (R_new%*%beta_R + k * (R_new[,2]^2-1) + k * (sin(R_new[,d2]) - exp(-0.5)*R_new[,d2]))
...
CV_GLasso = function(X, y, group_jj, cost_jj) {
  ...
  fit0 <- cv.sparsegl(X_sorted, y, group = group_sorted, asparse = 0, family = "gaussian", pf_group = cost_jj, standardize = FALSE)
  fit <- cv.sparsegl(X_sorted, y, group = group_sorted, asparse = 0, family = "gaussian", pf_group = cost_jj, standardize = FALSE, lambda = fit0$lambda)
...
HDQ_glasso_regimes = function(psi1,psi2,S1_new,J1s,J2s,L1,L2,C1s,C2s,parms,k)
{
  ...
  S2_new = get_S2(S1_new,a1_hat_); d2 = ncol(S2_new); 
  S_new = cbind(S1_new,a1_hat,S2_new); H2_new = cbind(1,S_new)
  a2_hat = as.vector(ifelse((H2_new%*%psi2) > ((C2s[2]-C2s[1])/2), 1, -1)); 
  a2_hat_ = a2_hat; a2_hat_[a2_hat_==-1] = 0;
  
  beta_Y1 = parms[[1]]; beta_R = parms[[2]]; beta_SR = parms[[3]];
  R_new = S2_new - S1_new - as.vector(a1_hat_); SR_new = cbind(S1_new,a1_hat_,R_new)
  y_test = SR_new%*%beta_SR + a1_hat_ * (S1_new %*% beta_Y1 + k * (S1_new[,1]^2-1)) + a2_hat_ * (R_new%*%beta_R + k * (R_new[,2]^2-1) + k * (sin(R_new[,d2]) - exp(-0.5)*R_new[,d2]))
...
RQL_glasso = function(S1,S2,A1,A2,y,JJ1,JJ2,CJJ1,CJJ2,Learners,Learners_ps)
{
  n = nrow(S1); d1 = ncol(S1); cf = sample(1:n,n/2)
  
  X2 = cbind(S1,A1,S2)
  g_hat1 = SuperLearner(Y = A2[cf], X = as.data.frame(X2[cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
  g_hat2 = SuperLearner(Y = A2[-cf], X = as.data.frame(X2[-cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
  ...
  g21 = SuperLearner(Y = A1[cf], X = as.data.frame(X1[cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
  g22 = SuperLearner(Y = A1[-cf], X = as.data.frame(X1[-cf,]), family = binomial(), SL.library = Learners_ps, method = "method.NNloglik")
...
RQL_gLasso_regimes <- function(alpha_hat,beta_hat,S1_new,J1s,J2s,L1,L2,C1s,C2s,parms,k)
{
  n_test = nrow(S1_new); d1 = ncol(S1_new); 
  ...
  beta_Y1 = parms[[1]]; beta_R = parms[[2]]; beta_SR = parms[[3]]
  R_new = S2_new - S1_new - as.vector(a1_hat); SR_new = cbind(S1_new,a1_hat,R_new);
  y_new = SR_new%*%beta_SR + a1_hat * (S1_new %*% beta_Y1 + k * (S1_new[,1]^2-1)) + a2_hat * (R_new%*%beta_R + k * (R_new[,2]^2-1) + k * (sin(R_new[,d2]) - exp(-0.5)*R_new[,d2]))
...
RQLo <- function(S1,S2,A1,A2,y,Learners,Learners_ps) 
{
  n = nrow(S1)
  cf = sample(1:n,n/2)
  ...
  beta_hat = as.vector(lm(impute_y2 ~ impute_x2)$coefficients) #the first element is intercept
  
  return(list(alpha_hat,beta_hat))
}

RQLo_regimes <- function(alpha_hat,beta_hat,S1_new,C1s,C2s,parms,k)
{
  n_test = nrow(S1_new); d1 = ncol(S1_new); 
  X1_new = cbind(1,S1_new)
  a1_hat = as.vector(ifelse((X1_new%*%beta_hat) > (C1s[2]-C1s[1]), 1, 0)); 
...
HDQo_regimes = function(psi1,psi2,S1_new,J1s,J2s,L1,L2,C1s,C2s,parms,k)
{
  n_test = nrow(S1_new); d1 = ncol(S1_new);
  ...
  a1_hat_ = a1_hat; a1_hat_[a1_hat_==-1] = 0;
  S2_new = get_S2(S1_new,a1_hat_); d2 = ncol(S2_new); 
  S_new = cbind(S1_new,a1_hat,S2_new); H2_new = cbind(rep(1,n_test),S_new)
  a2_hat = as.vector(ifelse((H2_new%*%psi2) > ((C2s[2]-C2s[1])/2), 1, -1)); 
  a2_hat_ = a2_hat; a2_hat_[a2_hat_==-1] = 0;
