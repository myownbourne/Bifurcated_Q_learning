...
BQL_regimes_4 <- function(alphas,betas,gammas,deltas,S1_new,J1s,J2s,L1,L2,C1s,CJ1s,C2s,CJ2s,parms,BQL_gold)
{
  alphas_gold = BQL_gold[[1]]
  betas_gold = BQL_gold[[2]]
  gammas_gold = BQL_gold[[3]]
  deltas_gold = BQL_gold[[4]]
  ...
  for(i in 1:n_test){
    ...
    SL1_delta_gold = rep(0,length(J1s))
    for(k1 in 1:length(J1s)){
      delta_hat_k1 = deltas[[k1]]$delta_hat_k1
      SL1_delta[k1] = c(1,SL1i)%*%delta_hat_k1
      delta_hat_k1_gold = deltas_gold[[k1]]$delta_hat_k1
      SL1_delta_gold[k1] = c(1,SL1i)%*%delta_hat_k1_gold
    }
    J1_hati = which.max(SL1_delta)
    J1_goldi = which.max(SL1_delta_gold)
    J1_hat[i] = J1_hati
    
    cov1_hat_i  = J1s[[J1_hati]]
    cov1_gold_i = J1s[[J1_goldi]]
    
    d1   = ncol(S1_new)
    full = setdiff(1:d1, L1)

    if (length(cov1_gold_i) > 0) {
      tpr1[i] <- length(intersect(cov1_hat_i, cov1_gold_i)) / length(cov1_gold_i)
    } else {
      tpr1[i] <- NA_real_
    }
    
    neg1_i <- setdiff(full, cov1_gold_i)
    if (length(neg1_i) > 0) {
      fpr1[i] <- length(intersect(cov1_hat_i, neg1_i)) / length(neg1_i)
    } else {
      fpr1[i] <- NA_real_
    }
  ...
  for(i in 1:n_test){
    ...
    SL2b_beta_gold = rep(0,length(J2s))
    for(k2 in 1:length(J2s)){
      beta_hat_0k1k2 = betas[[J1_hati]][[k2]][[1]]$beta_hat_a1k1k2
      beta_hat_1k1k2 = betas[[J1_hati]][[k2]][[2]]$beta_hat_a1k1k2
      
      beta_hat_0k1k2_gold = betas_gold[[J1_hati]][[k2]][[1]]$beta_hat_a1k1k2
      beta_hat_1k1k2_gold = betas_gold[[J1_hati]][[k2]][[2]]$beta_hat_a1k1k2
      
      if(a1_hat[i]==0){
        SL2b_beta[k2] = c(1,SL2ib)%*%beta_hat_0k1k2
        SL2b_beta_gold[k2] = c(1,SL2ib)%*%beta_hat_0k1k2_gold
      }else{
        SL2b_beta[k2] = c(1,SL2ib)%*%beta_hat_1k1k2
        SL2b_beta_gold[k2] = c(1,SL2ib)%*%beta_hat_1k1k2_gold
      }
    }
    J2_hati = which.max(SL2b_beta)
    J2_goldi = which.max(SL2b_beta_gold)
    J2_hat[i] = J2_hati
    
    
    cov2_hat_i  = J2s[[J2_hati]]
    cov2_gold_i = J2s[[J2_goldi]]
    
    d2   = ncol(S2_new)
    full2 = setdiff(1:d2, L2)
    
    if (length(cov2_gold_i) > 0) {
      tpr2[i] <- length(intersect(cov2_hat_i, cov2_gold_i)) / length(cov2_gold_i)
    } else {
      tpr2[i] <- NA_real_
    }
    
    neg2_i <- setdiff(full2, cov2_gold_i)
    if (length(neg2_i) > 0) {
      fpr2[i] <- length(intersect(cov2_hat_i, neg2_i)) / length(neg2_i)
    } else {
      fpr2[i] <- NA_real_
    }
  ...
  return(list(J1_hat,a1_hat,J2_hat,a2_hat,y_new,tpr1,fpr1,tpr2,fpr2))
}
...
HDQ_glasso_regimes = function(psi1,psi2,S1_new,J1s,J2s,L1,L2,C1s,C2s,parms,CJ1s,CJ2s,BQL_gold)
{
  alphas_gold = BQL_gold[[1]]
  betas_gold = BQL_gold[[2]]
  gammas_gold = BQL_gold[[3]]
  ...
  CJ1 = CJ1s[J_psi1]; J1_f = J_psi1[which.min(CJ1)];
  CJ2 = CJ2s[J_psi22]; J2_f = J_psi22[which.min(CJ2)];
  ...
  SL2 = as.matrix(S2_new[,L2])
  for(i in 1:n_test){
    SL2ib = c(SL1[i,],S1_new[i,J1s[[J1_f]]],SL2[i,])
    SL2b_beta_gold = rep(0,length(J2s))
    for(k2 in 1:length(J2s)){
      beta_hat_0k1k2_gold = betas_gold[[J1_f]][[k2]][[1]]$beta_hat_a1k1k2
      beta_hat_1k1k2_gold = betas_gold[[J1_f]][[k2]][[2]]$beta_hat_a1k1k2
  ...
  return(list(a1_hat_, a2_hat_, y_test, J1_f, J2_f, tpr1,fpr1,tpr2,fpr2))
}
...
RQL_gLasso_regimes <- function(alpha_hat,beta_hat,S1_new,J1s,J2s,L1,L2,C1s,C2s,CJ1s,CJ2s,parms,BQL_gold)
{
  alphas_gold = BQL_gold[[1]]
  betas_gold = BQL_gold[[2]]
  gammas_gold = BQL_gold[[3]]
  ...
  CJ1 = CJ1s[J_ab1]; J1_f = J_ab1[which.min(CJ1)];
  CJ2 = CJ2s[J_a2];  J2_f = J_a2[which.min(CJ2)];
  ...
  return(list(a1_hat,a2_hat,y_new,J1_f, J2_f, tpr1,fpr1,tpr2,fpr2))
}

RQLo <- function(S1,S2,A1,A2,y,Learners) 
{
  n = nrow(S1)
  cf = sample(1:n,n/2)
...
RQLo_regimes <- function(alpha_hat,beta_hat,S1_new,C1s,C2s,parms,L1,L2,J1s,J2s,CJ1s,CJ2s,BQL_gold)
{
  alphas_gold = BQL_gold[[1]]
  betas_gold = BQL_gold[[2]]
  gammas_gold = BQL_gold[[3]]
...
HDQo_regimes = function(psi1,psi2,S1_new,J1s,J2s,L1,L2,C1s,C2s,parms,CJ1s,CJ2s,BQL_gold)
{
  alphas_gold = BQL_gold[[1]]
  betas_gold = BQL_gold[[2]]
  gammas_gold = BQL_gold[[3]]
