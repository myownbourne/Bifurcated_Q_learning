...
sim_result = array(data = 0, dim = c(num_n, 5, num_sim))
...
sim_result = foreach(i = 1:num_sim, .combine = 'c') %dopar% {
  ...
  library(ncvreg)
  set.seed(2025*i) 
  sim_resulti = matrix(data = 0, nrow = num_n, ncol = 5)
...
stopCluster(cl)
sim_result = array(unlist(sim_result), dim = c(num_n, 5, num_sim))
...
write.csv(long_format_data, file = "result_S11n_time.csv", row.names = FALSE)
